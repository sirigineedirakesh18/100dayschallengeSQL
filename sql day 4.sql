...
*/
select gender,cast(AVG(total_amount) AS numeric(10,2)) as avg_amount from
customers c 
inner join
orders o on  c.customer_id=o.customer_id group by gender
...
*/
select DATENAME(MONTH, order_date) as month_name, sum(quantity*price_per_unit) as sales_revenue from sales where year(order_date)=2023 group by DATENAME(MONTH, order_date) 
